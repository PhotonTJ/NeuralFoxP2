...
from neural_foxp2 import (
    get_language,
    load_model_and_tokenizer,
    load_matched_prompts,
    build_language_token_sets,
    run_stage1,
    run_stage2,
    run_stage3
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="hindi",
        help="Target language for steering (hindi, spanish, chinese, bengali, telugu)"
    )
...
def main():
    ...
    print("="*70)
    print(f"Target Language: {config.target_language.capitalize()}")
    print(f"Layer range: {config.layer_range}")
    print(f"N prompts: {config.n_prompts}")
    print(f"SAE epochs: {config.epochs}")
    print("="*70)
    
    language_config = get_language(config.target_language)
    
    # Load model and tokenizer
    model, tokenizer = load_model_and_tokenizer(hf_token=config.hf_token)    
    # Load data
    matched_prompts = load_matched_prompts(language_config)
    V_tgt, V_en = build_language_token_sets(tokenizer, language_config)
